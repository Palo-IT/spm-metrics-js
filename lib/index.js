...
function SpmCustomMetrics (token, metricsInterval, metricsApiEndpoint) {
  ...
  this.metricsUrl = `${ELK_HOST}:${ELK_PORT}`
...
SpmCustomMetrics.prototype.sendEvent = function (type, event, callback) {
  ...
  let path
  if (event.container_id && event.container_name){
    path = 'container-logs/' + event.container_name
  } else if (event && event.creator){
    path =  'system-logs/' + event.creator
  }
  ...
  var options = {
    // url: (`http://${ELK_USERNAME}:${ELK_PASSWORD}@${ELK_HOST}:${ELK_PORT}/` + originApp +'/' + originHost ),
    url: `https://${ELK_USERNAME}:${ELK_PASSWORD}@${ELK_HOST}:${ELK_PORT}/${path}/_bulk`,
    headers: {
      'User-Agent': 'spm-metrics-js',
      'Content-Type': 'application/json'
    // 'Keep-Alive': false

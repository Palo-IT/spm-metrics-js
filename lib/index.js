...
SpmCustomMetrics.prototype.sendEvent = function (type, event, callback) {
  ...
  let originHost
  let originApp
  if (event.container_id) { // from docker
    originHost = event.container_id
    originApp = event.container_name
  } else if (event.creater) { // from vm
    originHost = event.creater
    originApp = 'system'
  } else {
    originHost = 'unknown'
    originApp = 'unknown'
  ...
  var options = {
    url: (`http://${ELK_USERNAME}:${ELK_PASSWORD}@${ELK_HOST}:${ELK_PORT}/` + originHost +'/' + originApp ),
